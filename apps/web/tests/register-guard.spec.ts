...
test("register redirects signed-in users to the dashboard", async ({ page }) => {
  test.skip(!process.env.DATABASE_URL, "Set DATABASE_URL to run the database-backed e2e path.");

  const email = `e2e-register-guard-${Date.now()}@example.com`;
  const response = await page.request.post("/api/auth/register", {
    data: { email, name: "E2E Register Guard User", password: "password-123" },
