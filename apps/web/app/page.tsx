...
import { ArrowRight, Check, FileText, Gauge, ScanLine, Target } from "lucide-react";
...
const jsonLd = {
  ...
  description:
    "Build role-tailored resumes, cover letters, and career documents with surgical precision.",
};
  ...
  return (
    <main className="signal-site min-h-screen bg-[#f3f3f3] pt-[52px] text-[#123c3a] sm:pt-[60px]">
      ...
      <AppHeader
        actions={
          <nav className="flex items-center gap-2" aria-label="Main navigation">
            <Link
              href="/dashboard"
              className="inline-flex min-h-11 items-center rounded-full px-4 py-2 text-sm font-bold transition hover:bg-[#123c3a] hover:text-[#b9ff66] sm:px-5"
            >
              Dashboard
            </Link>
            <Link
              href="/builder"
              className="signal-button-dark whitespace-nowrap text-xs sm:text-sm"
            >
              Open builder
            </Link>
          </nav>
      ...
      >
        <Link
          href="/"
          className="font-signal text-xl font-black tracking-[-0.08em] transition hover:text-[#3a7a1a] sm:text-2xl"
          aria-label="CareerLaunch Studio Homepage"
        >
          CareerLaunch
        </Link>
      </AppHeader>

      <section className="relative overflow-hidden px-5 pb-20 pt-14 lg:pb-28 lg:pt-24" aria-labelledby="hero-heading">
          ...
          <div className="relative z-10">
            <div className="inline-flex w-fit items-center gap-2 rounded-full border border-[#123c3a]/10 bg-white px-3 py-2 shadow-sm">
              <span className="relative flex h-2 w-2" aria-hidden="true">
                ...
                <span className="relative inline-flex h-2 w-2 rounded-full bg-[#3a7a1a]" />
              </span>
              <span className="text-xs font-black uppercase tracking-[0.18em] text-[#3d3d3d]">
                The future of career architecture
              </span>
            </div>

            <h1 id="hero-heading" className="font-signal mt-7 max-w-5xl text-[clamp(3.2rem,9vw,8.5rem)] font-black leading-[0.82] tracking-[-0.08em]">
              Unfair resume.
            </h1>
            <p className="mt-7 max-w-xl text-lg font-medium leading-8 text-[#3d3d3d] lg:text-xl">
            ...
            <div className="mt-7 flex items-center gap-2 text-sm font-semibold text-[#3d3d3d]">
              <span className="h-2 w-2 rounded-full bg-[#b9ff66] shadow-[0_0_0_6px_rgba(185,255,102,0.18)]" aria-hidden="true" />
          ...
          <div className="relative z-10 flex justify-center lg:justify-end">
            <div className="resume-card-float relative w-full max-w-[500px] rotate-[-3deg] rounded-[28px] border border-[#123c3a]/10 bg-white p-7 shadow-[0_28px_70px_rgba(18,60,58,0.14)] transition duration-500 hover:rotate-0">
              ...
              <div className="absolute -bottom-6 left-8 flex items-center gap-3 rounded-2xl border border-[#123c3a]/10 bg-white px-4 py-3 shadow-xl">
                <span className="relative h-2.5 w-2.5" aria-hidden="true">
                ...
                <span className="font-mono text-xs font-black text-[#123c3a]">Profile activity high</span>
              </div>

              <div className="flex items-start gap-5">
                <div className="grid h-20 w-20 place-items-center rounded-full bg-gradient-to-br from-[#b9ff66] to-[#72d8ff] p-1">
                  <div className="grid h-full w-full place-items-center rounded-full bg-white font-signal text-2xl font-black text-[#123c3a]">CL</div>
                </div>
                <div className="min-w-0 flex-1">
                  <p className="font-signal text-3xl font-black tracking-[-0.06em] text-[#123c3a]">Jordan Smith</p>
                  <p className="mt-1 text-sm font-black text-[#2e6314]">Customer Success Lead</p>
                  <p className="mt-2 text-xs font-semibold text-[#3d3d3d]">Career switcher / New York, NY</p>
                </div>
              </div>

              <div className="mt-7 flex flex-wrap gap-2">
                {["Retention", "CRM", "SQL", "Onboarding"].map((skill) => (
                  <span key={skill} className="rounded-full border border-transparent bg-[#f3f3f3] px-3 py-1.5 text-xs font-black text-[#123c3a] transition hover:border-[#b9ff66]">
                    {skill}
                  </span>
                ))}
              </div>

              <div className="mt-7 rounded-2xl bg-[#f3f3f3] p-5 text-sm font-medium leading-7 text-[#3d3d3d]">
              ...
              <div className="mt-7 grid grid-cols-3 gap-4 border-t border-[#123c3a]/10 pt-6">
                {signalStats.map(([label, value]) => (
                  <div key={label}>
                    <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#555]">{label}</p>
                    <p className="font-signal mt-1 text-xl font-black tracking-[-0.05em] text-[#123c3a]">{value}</p>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section id="platform" className="bg-[#f3f3f3] px-5 py-20" aria-labelledby="platform-heading">
        <div className="mx-auto max-w-7xl">
          <div className="mb-12 flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
            <h2 id="platform-heading" className="font-signal max-w-3xl text-4xl font-black leading-[0.95] tracking-[-0.07em] md:text-6xl text-[#123c3a]">
              Design your trajectory with surgical precision.
            </h2>
            <p className="max-w-md text-base font-medium leading-7 text-[#3d3d3d]">
            ...
            {platformLayers.map((layer) => (
              <article key={layer.label} className={`${layer.wide ? "lg:col-span-12" : "lg:col-span-6"} group rounded-[30px] border border-[#123c3a]/10 bg-white p-7 shadow-sm transition hover:border-[#b9ff66]`}>
                <div className={layer.wide ? "grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center" : "flex h-full flex-col"}>
                  <div>
                    <span className="inline-flex rounded-lg border border-[#123c3a]/10 bg-[#f7f7f7] px-3 py-1 font-mono text-xs font-black uppercase text-[#555]">{layer.label}</span>
                    <h3 className="font-signal mt-5 text-3xl font-black tracking-[-0.05em] text-[#123c3a]">{layer.title}</h3>
                    <p className="mt-4 max-w-2xl text-base font-medium leading-7 text-[#3d3d3d]">{layer.text}</p>
                  </div>
                  <div className={`${layer.wide ? "mt-0" : "mt-8"} rounded-2xl border border-[#123c3a]/10 bg-[#123c3a] p-5 text-white`}>
                    <div className="mb-4 flex items-center justify-between text-xs font-black uppercase tracking-[0.18em] text-white/80">
                      <span>Signal scan</span>
                      <ScanLine size={18} className="text-[#b9ff66]" aria-hidden="true" />
                    </div>
                    ...
                    <div className="mt-5 grid grid-cols-2 gap-3">
                      <div className="rounded-xl border border-dashed border-white/40 p-4 text-center font-mono text-[10px] uppercase text-white/90">ATS scan</div>
      ...
      <section className="bg-[#123c3a] px-5 py-20 text-white" aria-labelledby="process-heading">
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
          <div>
            <p className="font-mono text-xs font-black uppercase tracking-[0.22em] text-[#b9ff66]">Process</p>
            <h2 id="process-heading" className="font-signal mt-4 text-5xl font-black leading-[0.9] tracking-[-0.07em]">Three stages to a cleaner career signal.</h2>
          </div>
          <div className="grid gap-5 md:grid-cols-3">
            {[
              ...
              return (
                <div key={String(title)} className="rounded-[24px] border border-white/10 bg-white/[0.06] p-6">
                  <div className="grid h-14 w-14 place-items-center rounded-full bg-[#b9ff66] text-[#123c3a]">
                    <DisplayIcon size={24} aria-hidden="true" />
                  </div>
                  <h3 className="font-signal mt-6 text-2xl font-black tracking-[-0.05em]">{String(title)}</h3>
                  <p className="mt-2 text-sm font-medium leading-6 text-white/85">{String(text)}</p>
                </div>
              );
            })}
          </div>
      ...
      <section className="bg-[#b9ff66] px-5 py-10 text-[#123c3a] sm:py-12" aria-labelledby="cta-heading">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <h2 id="cta-heading" className="font-signal text-3xl font-black leading-[0.95] tracking-[-0.07em] sm:text-4xl md:text-6xl">
          ...
          <Link href="/builder" className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-[#123c3a] px-5 text-sm font-black text-white shadow-[0_4px_0_#072f2c] transition hover:-translate-y-0.5 sm:w-auto sm:min-h-14 sm:px-7">
            Build my resume <Check size={18} aria-hidden="true" />
          </Link>
        </div>
      </section>

      <footer className="border-t border-[#123c3a]/10 px-5 py-6 text-center text-xs font-medium text-[#3d3d3d]">
