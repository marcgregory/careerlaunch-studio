...
export function EmptyState({ variant = "no-resumes", searchTerm, activeFilter }: EmptyStateProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-[30px] border border-dashed border-[#123c3a]/15 bg-white/60 p-10 text-center">
      <div className="grid h-14 w-14 place-items-center rounded-full bg-[#f3f3f3] text-[#4b4b4b]">
        <Search size={24} />
      </div>
      <h3 className="font-signal mt-5 text-xl font-black tracking-[-0.04em] text-[#123c3a]">
        No resumes found
      </h3>
      <p className="mt-2 max-w-sm text-sm font-medium leading-6 text-[#4b4b4b]">
        {searchTerm && (
